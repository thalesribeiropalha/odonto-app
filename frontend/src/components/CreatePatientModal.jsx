...
const CreatePatientModal = ({ isOpen, onClose, onPatientCreated }) => {
  ...
  const [currentTab, setCurrentTab] = useState('basic'); // basic, address, medical, emergency
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setErrors({ submit: response.message || 'Erro ao criar paciente' });
      }
    } catch (error) {
      console.error('Erro ao criar paciente:', error);
      setErrors({ submit: error.message || 'Erro ao criar paciente' });
    } finally {
      setLoading(false);
    }
  };
